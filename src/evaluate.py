...
def run_yolov7_test(yolov7_dir: Path, weights: Path, data_yaml: Path, img_size: int, 
                    batch_size: int, conf_thres: float, iou_thres: float):
...
def measure_inference_latency(yolov7_dir: Path, weights: Path, img_size: int, 
                               num_iterations: int = 100, device: str = "0"):
...
def main():
    ...
    parser.add_argument("--measure-latency", action="store_true", 
                        help="Measure inference latency (takes time)")
    ...
    run_yolov7_test(args.yolov7_dir, args.weights, args.data, args.img_size,
                    args.batch_size, args.conf_thres, args.iou_thres)
    ...
    if args.measure_latency:
        ...
        latency = measure_inference_latency(args.yolov7_dir, args.weights, args.img_size,
                                            args.latency_iterations, args.device)
